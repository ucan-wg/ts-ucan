...
const [ encodedHeader, encodedPayload, encodedSignature ] = oldUcan.split(".")
const signature = encodedSignature
const header = JSON.parse(uint8arrays.toString(uint8arrays.fromString(encodedHeader, "base64url")))
const payload = JSON.parse(uint8arrays.toString(uint8arrays.fromString(encodedPayload, "base64url")))

describe("compatibility", () => {

  beforeAll(loadTestPlugins)

  it("allows parsing UCANs with 'uav: 1.0.0' into 'ucv: 0.3.0'", async () => {
